...
public interface ProjectFileRepository extends JpaRepository<ProjectFile, Long> {
    ...
    @Query("""
            SELECT new com.contexto.file.FileSnapshot(f.id, f.relativePath, f.contentHash)
            FROM ProjectFile f
    ...
    @Query("""
            SELECT new com.contexto.file.ProjectFileSummaryDTO(
    ...
    @Query("""
            SELECT new com.contexto.file.FileContent(f.id, f.relativePath, f.language, f.content)
            FROM ProjectFile f
            WHERE f.id IN :ids
            """)
